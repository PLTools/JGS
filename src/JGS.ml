  ...
  conde
    [
      ...
      fresh (l_hd l_tl res_hd res_tl)
        (l === l_hd % l_tl)
        (res === res_hd % res_tl)
        (f l_hd res_hd) (mapo f l_tl res_tl);
    ]
...
module Polarity = struct
  [%%distrib
  type nonrec ground = Extends | Super
...
module CC_type = struct
  ...
  type nonrec 'id ground =
    ...
    | CC_var of
        ...
        * Std.Nat.ground
        * 'id CC_subst.ground
        * 'id Jtype.ground Std.Option.ground
  [@@deriving gt ~options:{ show; fmt; gmap }]]
...
module Verifier (CT : CLASSTABLE) : VERIFIER = struct
  let params : int ilogic -> int ilogic Jtype.injected Std.List.injected -> goal
      =
   fun id p ->
    fresh decl (CT.decl_by_id id decl)
    ...
    conde
      [
        ...
        fresh (param params_val)
          ...
          (cc_targ
          === cc_var (CT.new_var ()) i (cc_subst param) (Std.some (null ())))
          (params id params_val) (ntho params_val i param);
        fresh (t subst params_val)
          (targ === wildcard (Std.some (Std.pair (super ()) t)))
          (cc_targ === cc_var (CT.new_var ()) i (cc_subst subst) (Std.some t))
          (params id params_val) (ntho params_val i subst);
        fresh (t t2 params_val)
          (targ === wildcard (Std.some (Std.pair (extends ()) t)))
          (cc_targ
          === cc_var (CT.new_var ()) i (cc_inter t t2) (Std.some (null ())))
          (params id params_val) (ntho params_val i t2);
      ]
